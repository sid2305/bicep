    ...
    public class BicepDocumentLinkHandler : DocumentLinkHandlerBase
    {
        ...
        public static IEnumerable<DocumentLink> GetDocumentLinks(IModuleDispatcher moduleDispatcher, DocumentLinkParams request, CancellationToken cancellationToken)
        {
            ...
            if (request.TextDocument.Uri.Scheme == LangServerConstants.ExternalSourceFileScheme)
            {
                ...
                if (externalReference is not null && !string.IsNullOrEmpty(externalReference.RequestedFile)/*asdfg or empty - why?*/)
                {
                    ...
                    if (externalReference.ToArtifactReference().IsSuccess(out var artifactReference, out message))
                    {
                        ...
                        if (sourceArchiveResult.SourceArchive is { } archive)
                        {
                            if (archive.SourceFiles.FirstOrDefault(f => f.Path == externalReference.RequestedFile/*asdfgasdfg extract*/) is { } sourceFile &&
                                sourceArchiveResult.SourceArchive.DocumentLinks.TryGetValue(externalReference.RequestedFile, out var links))
                            {

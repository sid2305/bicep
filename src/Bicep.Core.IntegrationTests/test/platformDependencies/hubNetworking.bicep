...
module modBastionPublicIp '../platformDependencies/publicIp.bicep' = if (parAzBastionEnabled) {
...
module modGatewayPublicIp '../platformDependencies/publicIp.bicep' = [for (gateway, i) in varGwConfig: if ((gateway.name != 'noconfigVpn') && (gateway.name != 'noconfigEr')) {
...
module modAzureFirewallPublicIp '../platformDependencies/publicIp.bicep' = if (parAzFirewallEnabled) {
...
module modAzureFirewallMgmtPublicIp '../platformDependencies/publicIp.bicep' = if (parAzFirewallEnabled && (contains(map(parSubnets, subnets => subnets.name), 'AzureFirewallManagementSubnet'))) {
...
module modPrivateDnsZones '../platformDependencies/privateDnsZones.bicep' = if (parPrivateDnsZonesEnabled) {
...
module modCustomerUsageAttribution '../commonDependencies/cuaIdResourceGroup.bicep' = if (!parTelemetryOptOut) {
  #disable-next-line no-loc-expr-outside-params //Only to ensure telemetry data is stored in same location as deployment. See https://github.com/Azure/ALZ-Bicep/wiki/FAQ#why-are-some-linter-rules-disabled-via-the-disable-next-line-bicep-function for more information
  name: 'pid-${varCuaid}-${uniqueString(resourceGroup().location)}'
  params: {}
}

module modCustomerUsageAttributionZtnP1 '../commonDependencies/cuaIdResourceGroup.bicep' = if (!parTelemetryOptOut && varZtnP1Trigger) {
  #disable-next-line no-loc-expr-outside-params //Only to ensure telemetry data is stored in same location as deployment. See https://github.com/Azure/ALZ-Bicep/wiki/FAQ#why-are-some-linter-rules-disabled-via-the-disable-next-line-bicep-function for more information

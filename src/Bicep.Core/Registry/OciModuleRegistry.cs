...
namespace Bicep.Core.Registry
{
    public sealed class OciModuleRegistry : ExternalArtifactRegistry<OciModuleReference, OciArtifactResult>
    {
        ...
        public override async Task<bool> CheckArtifactExists(OciModuleReference reference)
        {
            ...
            {
                // Try getting module
                // CONSIDER: No need to download the entire artifact including all layers to check if it exists.
        ...
        private CachedLayerInfo[] GetCachedLayerInfos(OciModuleReference ociArtifactModuleReference, string? mediaType = null)
        {
            if (features.PublishSourceEnabled)
            {
                OciManifest ociManifest = GetCachedManifest(ociArtifactModuleReference);
                var layers = ociManifest.Layers.Select((l, i) => new CachedLayerInfo(i, l.MediaType, l.Annotations));
                if (mediaType is { })
                {
                    layers = layers.Where(l => BicepMediaTypes.MediaTypeComparer.Equals(l.MediaType, mediaType));
                }
            ...
            {
                return Array.Empty<CachedLayerInfo>();
            }
        }
        ...
        {
            // Write out an empty config for now asdfg
        ...
        protected override void WriteArtifactContentToCache(OciModuleReference reference, OciArtifactResult result)
        {
            ...
            // TODO: IsArtifactRestoreRequired assumes there must be a ModuleMain file, which isn't true for provider artifacts //asdfg don't write main.json
            ...
            if (this.features.PublishSourceEnabled)
            {
                // Write all layers as separate binary files.  We do this rather than writing individual files we know about,
                //   (e.g. "source.zip") because this way we can restore all layers even if we don't know what they're for.
                ...
                {
                    var layerPath = GetLayerFilePath(reference, iLayer);
                    File.WriteAllBytes(layerPath, layer.Data.ToArray());
                    ++iLayer;
                }
            }
        }
        ...
        private string GetModuleFilePath(OciModuleReference reference, ModuleFileType fileType)
        {
            var fileName = fileType switch
            {
                ...
                ModuleFileType.LayerBaseName => "layer", // layers are named "layer.0", "layer.1", etx
        ...
        private string GetLayerFilePath(OciModuleReference reference, int layerIndex)
        {
            return $"{GetModuleFilePath(reference, ModuleFileType.LayerBaseName)}.{layerIndex}";
        }

        public override SourceArchive? TryGetSourceArchive(OciModuleReference reference)
        {
            if (features.PublishSourceEnabled)
            {
                var sourceLayer = GetCachedLayerInfos(reference, BicepMediaTypes.BicepSourceV1Layer).SingleOrDefault();
                if (sourceLayer is { })
                {
                    var zipFile = GetLayerFilePath(reference, sourceLayer.Index);
        ...
        private enum ModuleFileType
        {
            ...
            Lock,
            Metadata,
            Provider, // TODO: this should instead read directly out of the layer binary files like we do for source codea
            LayerBaseName,
        };

        private record CachedLayerInfo(
            int Index,
            string MediaType,

...
public class OciRegistryContentClient : IOciRegistryContentClient
{
    ...
    public async Task<IEnumerable<(string artifactType, string digest)>> GetReferrersAsync(string mainManifestDigest)
    {
        ...
        if (_client.Pipeline is { } pipeline) // asdfg this guards against Bicep.Core.UnitTests.Registry.MockRegistryBlobClient which doesn't implement Pipeline.  Should it be implemented in tests?
        {

...
namespace Bicep.Core.Registry.Oci
{
    ...
    public class OciArtifactReferenceParts : IOciArtifactReferenceParts
    {
        public OciArtifactReferenceParts(string registry, string repository, string? tag, string? digest)
        {
            switch (tag, digest)
            {
                case (null, null):
        ...
        public override bool Equals(object? obj) //asdfg testing and for Equals of OciArtifactReference
        {
            if (obj is not OciArtifactReferenceParts other)
            {
                return false;
            }
...
//         public static ResultWithDiagnostic<OciArtifactReference> TryParseFullyQualified(ArtifactType type, string rawValue, RootConfiguration configuration, Uri parentModuleUri)
//         {
